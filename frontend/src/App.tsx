...
export default function App() {
  const [code, setCode] = useState(example),
    ...
    [title, setTitle] = useState('Duplicate finder')
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem('reviewlens-progress')
  ...
  function track(lang: string) {
    setProgress((old) => {
      const next = {
        ...
        streak: Math.max(1, old.streak),
        xp: old.xp + 25,
        languages: [...new Set([...old.languages, lang])],
      }
      localStorage.setItem('reviewlens-progress', JSON.stringify(next))
      return next
    })
  }
  ...
  return (
    <div className="min-h-screen">
      <nav className="sticky top-0 z-20 border-b border-white/10 bg-ink/80 px-5 py-4 backdrop-blur">
      ...
      <main id="top">
        <section className="relative overflow-hidden px-5 pb-20 pt-24">
          ...
          <div className="orb orb-cyan -right-20 top-40 h-80 w-80" />
          <div className="grid-bg absolute inset-0" />
          <div className="relative mx-auto max-w-5xl text-center">
            <motion.p
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="mb-6 inline-flex items-center gap-2 rounded-full border border-violet-300/25 bg-violet-300/10 px-4 py-1.5 text-sm text-violet-200 backdrop-blur"
            >
              ✦ Learn faster. Ship safer code.
            </motion.p>
            <motion.h1
            ...
            >
              Your AI-powered <span className="gradient-text">code review</span>
              <br />
              learning companion.
            </motion.h1>
            <p className="mx-auto mt-6 max-w-2xl text-lg leading-8 text-slate-400">
              Understand every mistake, practice challenges, and turn feedback into developer
              confidence.
            </p>
            <div className="mt-9 flex flex-wrap items-center justify-center gap-4">
              ...
              <a
                href="#ml-lab"
                className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-6 py-3 font-semibold text-slate-200 transition hover:border-brand/40 hover:bg-white/10"
              >
                Explore the ML lab
              </a>
            </div>
            <div className="mx-auto mt-14 grid max-w-2xl grid-cols-3 gap-3">
              <StatCard value="4+" label="Languages" icon={<Code2 size={16} />} />
              <StatCard value="100%" label="AI-powered" icon={<Sparkles size={16} />} />
              <StatCard value="Free" label="No credit card" icon={<LockKeyhole size={16} />} />
            </div>
          </div>
        </section>
        <section className="mx-auto grid max-w-6xl gap-4 px-5 py-12 md:grid-cols-3">
          {[
            ['Deep analysis', 'Find bugs, risks, and hidden edge cases.', '🚀'],
            ['Interactive fixes', 'Compare before and after code with clear explanations.', '🛠️'],
            ['Grow daily', 'Earn XP and work through focused coding challenges.', '📈'],
          ].map(([h, p, e]) => (
            <div key={h} className="glass glass-hover rounded-2xl p-6">
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-violet-500/25 to-cyan-400/20 text-lg">
                {e}
              </div>
              <h3 className="mt-4 font-display font-semibold">{h}</h3>
              <p className="mt-2 text-sm leading-6 text-slate-400">{p}</p>
            </div>
          ))}
        </section>
        <section id="workspace" className="mx-auto max-w-7xl px-5 pb-20">
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="font-display text-2xl font-bold">Review workspace</h2>
              <p className="mt-1 text-sm text-slate-400">
                Your personal coding coach, ready when you are.
              </p>
            </div>
            <div className="flex gap-2">
              <input
                ...
                className="w-40 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm outline-none focus:border-brand"
              ...
              <select
                ...
                className="rounded-lg border border-violet-300/20 bg-violet-300/10 px-3 py-2 text-sm text-violet-100"
              >
                <option value="balanced">Balanced mode</option>
                <option value="mentor">Mentor mode</option>
          ...
          <div className="grid gap-5 lg:grid-cols-[1.25fr_.9fr]">
            ...
            <div className="space-y-5">
              <ReviewPanel review={review as Review} reviewId={reviewId} />
              <Chat code={code} language={language} review={review} />
            </div>
          </div>
        ...
        <section className="border-y border-white/10 bg-brand/10 px-5 py-16 text-center">
          <h2 className="font-display text-3xl font-bold">
            Review with confidence. Learn by doing.
          </h2>
          <p className="mt-3 text-slate-300">Fast insight. Better code. Real progress.</p>
        </section>
      </main>
      <footer className="px-5 py-8 text-center text-sm text-slate-500">
        © 2026 ReviewLens · Built with FastAPI, React, Monaco, and AI.
      </footer>
    </div>
  )
}
...
function StatCard({ value, label, icon }: { value: string; label: string; icon: React.ReactNode }) {
  return (
    <div className="glass glass-hover rounded-2xl p-4 text-center">
      <span className="mx-auto mb-2 block w-fit rounded-lg bg-gradient-to-br from-violet-500/25 to-cyan-400/20 p-2 text-violet-200">
        {icon}
      </span>
